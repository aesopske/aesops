...
export function DiscussionBrowser({
    ...
    return (
        <div className='flex gap-6 items-start'>
            {/* Left — search + filters */}
            <aside className='w-72 shrink-0 sticky top-[4.5rem] space-y-4'>
                ...
                <DiscussionFilters
                    ...
                    onRecencyChange={(v) => setRecency(v === 'all' ? null : v)}
                    onSourceChange={(v) => setSource(v === 'all' ? null : v)}
                    onDatasetChange={(v) => setDataset(v || null)}
                    onSortChange={(v) => setSort(v === 'newest' ? null : v)}
                    onReset={reset}
                />
            </aside>
