...
export function DTypeBadge({ dtype }: { dtype: string }) {
    ...
    return (
        <span className={`rounded px-1.5 py-0.5 text-xs font-medium ${cls}`}>{dtype}</span>
    )
}
    ...
    return (
        <div>
            <div className='mb-3 flex flex-wrap gap-4 text-sm text-muted-foreground'>
                {meta.analyzedSheet && (
                    <span>Sheet: <span className='font-medium text-foreground'>{meta.analyzedSheet}</span></span>
                )}
                {meta.sheetNames && meta.sheetNames.length > 1 && (
                    <span className='rounded-full bg-secondary px-2 py-0.5 text-xs'>{meta.sheetNames.length} sheets</span>
                )}
            </div>
            <table className='w-full text-xs'>
                ...
                <tbody className='divide-y divide-border'>
                    {meta.columns.map((col) => <ColumnRow key={col.name} col={col} />)}
                </tbody>
            </table>
        </div>
    )
...
function ColumnRow({ col }: { col: ColumnStats }) {
    ...
    return (
        <tr className='align-top'>
            <td className='py-1.5 pr-3 font-medium text-foreground'>{col.name}</td>
            <td className='py-1.5 pr-3'><DTypeBadge dtype={col.dtype} /></td>
            <td className='py-1.5 pr-3 text-muted-foreground'>
                {col.nullPercent > 0
                    ? <span className='text-warning-foreground'>{col.nullPercent}%</span>
                    : '—'}
            </td>
            <td className='py-1.5 pr-3 text-muted-foreground'>{col.uniqueCount.toLocaleString()}</td>
            <td className='py-1.5 text-muted-foreground'>
                {isNumeric ? (
                    <span>
                        mean <span className='text-foreground'>{col.mean}</span>
                        {' · '}min <span className='text-foreground'>{col.min}</span>
                        {' · '}max <span className='text-foreground'>{col.max}</span>
                        {col.median !== undefined && (
                            <> · median <span className='text-foreground'>{col.median}</span></>
                        )}
                    </span>
        ...
        >
            <div className='p-4'>
                ...
                <div className='flex min-w-0 items-start gap-3'>
                    <div className={`shrink-0 rounded-lg p-2 ${isExcel ? 'bg-success/10 text-success' : 'bg-primary/10 text-primary'}`}>
