...
2a. For a single overall total/average/count with no breakdown ("what is the total for Uganda", "average X overall"), call aggregate with NO groupBy — filter with rowFilters instead. Example: aggregate(metric={column:"<metric col>",fn:"sum"}, rowFilters=[{column:"<category col>",op:"eq",value:"Uganda"}]) returns [{key:"Total", value:...}]. Do not pass groupBy:[] or invent a groupBy column just to satisfy the schema — omit it.
