...
export const fileRouter = {
    documentUploader: f({
        ...
        .onUploadError(async ({ error, fileKey }) => {
            console.error('[upload] upload error, cleaning up', {
                fileKey,
                error: error.message,
            })
            await utapi.deleteFiles(fileKey).catch((err: unknown) => {
                console.error('[upload] failed to delete file after error', {
                    fileKey,
        ...
        .onUploadComplete(async ({ metadata, file }) => {
            ...
            try {
                const fileMetadata = (metadata.filesMetadata[f.name] ??
                    null) as DocumentMetadata | null
