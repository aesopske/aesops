"""Run standalone:  uv run python -m src.cbk_card_transaction_values.validate
...
from src.cbk_card_transaction_values import fetch_raw
from src.cbk_card_transaction_values.parse import parse_response

# Hand-verified directly against a raw curl of the page (independent of parse.py), not by
# trusting the scraper's own output.
SPOT_CHECKS = {
    (2009, "November"): {
        "atm_cards_ksh_millions": 0.0, "prepaid_cards_ksh_millions": 66.7787,
        "charge_cards_ksh_millions": 1.2, "credit_cards_ksh_millions": 293.0,
        "debit_cards_ksh_millions": 22973.0, "pos_machines_ksh_millions": 3855.0,
        "total_ksh_millions": 23334.0,
    },
    (2026, "May"): {
        "atm_cards_ksh_millions": 0.0, "prepaid_cards_ksh_millions": 255.0,
        "charge_cards_ksh_millions": 0.0, "credit_cards_ksh_millions": 622.0,
        "debit_cards_ksh_millions": 34194.0, "pos_machines_ksh_millions": 25642.0,
        "total_ksh_millions": 35070.0,
    },
}
...
# The earliest 4 published months (this table starts July 2009) don't reconcile against
# `total = atm_cards + prepaid_cards + charge_cards + credit_cards + debit_cards` (values, Ksh
# Millions) — off by thousands, not rounding noise. Reconciles cleanly (within ordinary ~1
# Ksh-Million rounding, see the tolerance in check_total_reconciles) from November 2009 onward,
# with one further isolated outlier at 2023-09 (off by 60). A real, verified quirk in the
# source's data (the 2009 window mirrors the same period on the companion
# cbk_card_transactions dataset, though that one's non-reconciling window runs one month
# longer, through November) — see NOTES.md.
KNOWN_NON_RECONCILING_MONTHS = {
    (2009, "July"), (2009, "August"), (2009, "September"), (2009, "October"),
    (2023, "September"),
}
...
REQUIRED_FIELDS = [
    "year", "month", "atm_cards_ksh_millions", "prepaid_cards_ksh_millions",
    "charge_cards_ksh_millions", "credit_cards_ksh_millions", "debit_cards_ksh_millions",
    "pos_machines_ksh_millions", "total_ksh_millions",
]
CARD_TYPE_FIELDS = [
    "atm_cards_ksh_millions", "prepaid_cards_ksh_millions", "charge_cards_ksh_millions",
    "credit_cards_ksh_millions", "debit_cards_ksh_millions",
]
...
def check_total_reconciles(records: list[dict]) -> list[str]:
    ...
    for r in records:
        ...
        # Ordinary rounding noise across the dataset reaches ~1.2 Ksh Millions; 1.5 is a safe
        # margin that still catches the genuine 2023-09 outlier (delta 60) and the 2009 window
        # (deltas in the thousands).
        if abs(expected_total - r["total_ksh_millions"]) > 1.5:
            failures.append(
                f"{r['year']}-{r['month']}: total {r['total_ksh_millions']} != "
