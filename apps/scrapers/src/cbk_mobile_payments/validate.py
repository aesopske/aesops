"""Run standalone:  uv run python -m src.cbk_mobile_payments.validate
...
from src.cbk_mobile_payments import fetch_raw
...
SPOT_CHECKS = {
    (2007, "March"): {
        "active_agents": 307,
        "registered_accounts_millions": 0.020992,
        "agent_cash_in_out_volume_million": 0.021714,
        "agent_cash_in_out_value_ksh_billions": 0.0643905,
    },
    (2026, "May"): {
        "active_agents": 564_330,
        "registered_accounts_millions": 94.09,
        "agent_cash_in_out_volume_million": 214.33,
        "agent_cash_in_out_value_ksh_billions": 681.45,
    },
}
...
# 231 rows observed live (Mar 2007 - May 2026, ~231 months) at time of writing; range gives
# headroom for future months without needing to be updated on every scheduled run.
EXPECTED_RECORD_COUNT_RANGE = (200, 500)
REQUIRED_FIELDS = [
    "year", "month", "active_agents", "registered_accounts_millions",
    "agent_cash_in_out_volume_million", "agent_cash_in_out_value_ksh_billions",
]
...
def check_non_negative(records: list[dict]) -> list[str]:
    """Every metric here (agent counts, account counts, transaction volume/value) is a
    cumulative usage stat and should never be negative — unlike cbk_foreign_trade's trade
    balance or cbk_weighted_average_rates, there's no field here expected to go below zero."""
    failures = []
