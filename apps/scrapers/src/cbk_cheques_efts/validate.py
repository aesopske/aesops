"""Run standalone:  uv run python -m src.cbk_cheques_efts.validate
...
from src.cbk_cheques_efts import fetch_raw
from src.cbk_cheques_efts.parse import parse_response

# Hand-verified directly against a raw curl of the page (independent of parse.py), not by
# trusting the scraper's own output.
SPOT_CHECKS = {
    (2007, "October"): {
        "credit_efts_volume": 447_200,
        "credit_efts_value_ksh_billions": 54.741,
        "debit_cheques_volume": 1_523_853,
        "debit_cheques_value_ksh_billions": 295.953,
    },
    (2026, "May"): {
        "credit_efts_volume": 1_358_127,
        "credit_efts_value_ksh_billions": 74.57,
        "debit_cheques_volume": 1_193_536,
        "debit_cheques_value_ksh_billions": 194.44,
    },
}
...
# 227 rows observed live (Oct 2007 - May 2026, ~224 months) at time of writing; range gives
# headroom for future months without needing to be updated on every scheduled run.
EXPECTED_RECORD_COUNT_RANGE = (200, 500)
REQUIRED_FIELDS = [
    "year", "month", "credit_efts_volume", "credit_efts_value_ksh_billions",
    "debit_cheques_volume", "debit_cheques_value_ksh_billions",
]
