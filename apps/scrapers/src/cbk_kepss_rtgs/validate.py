"""Run standalone:  uv run python -m src.cbk_kepss_rtgs.validate
...
from src.cbk_kepss_rtgs import fetch_raw
from src.cbk_kepss_rtgs.parse import parse_response

# Hand-verified directly against a raw curl of the page (independent of parse.py), not by
# trusting the scraper's own output.
SPOT_CHECKS = {
    (2005, "August"): {"volume": 8_999, "value_ksh_millions": 496_250.0},
    (2026, "May"): {"volume": 936_787, "value_ksh_millions": 3_916_249.79},
}

# The source published a genuine conflicting duplicate for this (year, month) — two rows with
# different volume/value and no way to tell which is authoritative. parse.py keeps both rather
# than guessing; see NOTES.md. Any *other* duplicate key would indicate a real regression (or a
# new instance of this same source quirk worth re-checking by hand).
KNOWN_CONFLICTING_KEY = (2016, "January")
...
# 251 rows observed live (Aug 2005 - May 2026, ~250 months + 1 known conflict pair) at time of
# writing; range gives headroom for future months without needing to be updated on every
# scheduled run.
EXPECTED_RECORD_COUNT_RANGE = (220, 500)
REQUIRED_FIELDS = ["year", "month", "volume", "value_ksh_millions"]
